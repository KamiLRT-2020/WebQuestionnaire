...
function CreateQuestionBlock(legendText, videoURLArray, HumanLknessSliderIdArray, NaturalnessSliderIdArray, RoboticSliderIdArray, CalmSliderIdArray, RelaxSliderIdArray, GeneralCommentIdArray, methodString) {
    ...
    for (var i = 0; i < videoURLArray.length; i++) {
        ...
        intro1.innerHTML = "虚拟人物<b>" + methodString[i] + "</b>的视线动作与现实中的人接近吗?";
        container.appendChild(intro1);

        // add VAS slider
        container.appendChild(CreateSlider(HumanLknessSliderIdArray[i], "完全不接近", "非常接近"));

        // add explaination before the slider
        intro1 = document.createElement("li"); // use "li" to add a black dot before the text
        intro1.innerHTML = "虚拟人物<b>" + methodString[i] + "</b>的视线动作是否自然?";
        container.appendChild(intro1);

        // add VAS slider
        container.appendChild(CreateSlider(NaturalnessSliderIdArray[i],"非常不自然", "非常自然"));

        // add explaination before the slider
        intro1 = document.createElement("li"); // use "li" to add a black dot before the text
        intro1.innerHTML = "虚拟人物<b>" + methodString[i] + "</b>的视线动作是否像机器人?";
        container.appendChild(intro1);

        // add VAS slider
        container.appendChild(CreateSlider(RoboticSliderIdArray[i], "一点都不像机器人","非常像机器人"));

        // add explaination before the slider
        intro1 = document.createElement("li"); // use "li" to add a black dot before the text
        intro1.innerHTML = "虚拟人物<b>" + methodString[i] + "</b>看起来冷静还是紧张？";
        container.appendChild(intro1);

        // add VAS slider
        container.appendChild(CreateSlider(CalmSliderIdArray[i], "冷静", "紧张"));

        // add explaination before the slider
        intro1 = document.createElement("li"); // use "li" to add a black dot before the text
        intro1.innerHTML = "虚拟人物<b>" + methodString[i] + "</b>看起来放松还是兴奋？";
        container.appendChild(intro1);

        // add VAS slider
        container.appendChild(CreateSlider(RelaxSliderIdArray[i], "放松", "兴奋"));

        // add explaination before the comment block
        var intro1 = document.createElement("li"); // use "li" to add a black dot before the text
        intro1.innerHTML = "对虚拟人物" + methodString[i] + "的看法或评价?";
        container.appendChild(intro1);

        container.appendChild(CreateCommentBlock(GeneralCommentIdArray[i], "例如：我觉得她的表现很自然，因为.../很不自然，因为..."));
    }
...
function SetSubmitButton() {
    document.getElementById("submitButton").onclick = function () {
        ...
        else if (document.getElementById("name").value == "")
        {
            ...
            alert("请将结果文件发回给问卷发布者。"); // information to send back the result
        }
